...
The transition contract is `flat + long_entry -> pending_entry`, `pending_entry + P0.4A eligible later opportunity -> holding`, `holding + exit -> pending_exit`, and `pending_exit + P0.4A eligible later opportunity -> flat`. Opening creates one quantity-free `SimulatedPosition` with an authoritative `opened_at` time; closing removes it. Thus flat cannot have a position, holding/pending-exit has exactly one, and each pending state has exactly one compatible action. Equal-time opportunities remain ineligible because P0.4A remains authoritative. `LifecycleRecord` captures prior state, event identity/time, decision, reason, resulting state, and related action/position for reconstruction.
