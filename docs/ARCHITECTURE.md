...
Events sort by `(effective_time, explicit causal precedence, instrument identifier, event id)`. The precedence is market observation, feature, signal, then execution opportunity; it is a documented mapping rather than enum or input order. A signal becomes a price-free pending action. It is eligible only at the first opportunity for the same instrument whose event time is **strictly greater than** the signal availability time. Thus equal-time precedence makes the trace inspectable but cannot make an equal-time completed-bar close executable. `TimelineRecord` retains event, signal, opportunity, decision, and structured reason evidence. P0.4A creates no position, order, fill, price, cost, risk, or portfolio-accounting state; P0.4B may define lifecycle mechanics without weakening this causal boundary.
