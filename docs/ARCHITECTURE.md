...
Events sort by `(effective_time, explicit causal precedence, instrument identifier, event id)`. The precedence is market observation, feature, signal, then execution opportunity; it is a documented mapping rather than enum or input order. A signal becomes a price-free pending action. It is eligible only at the first opportunity for the same instrument whose event time is **strictly greater than** the signal availability time. Thus equal-time precedence makes the trace inspectable but cannot make an equal-time completed-bar close executable. `TimelineRecord` retains event, signal, opportunity, decision, and structured reason evidence. P0.4A itself creates no position, order, fill, price, cost, risk, or portfolio-accounting state; P0.4B composes this boundary without weakening it.
...
A legal `long_entry` while flat creates an immutable entry action and moves to `pending_entry`; only P0.4A's first strictly later eligible opportunity moves it to `holding`. A legal `exit` while holding creates an exit action and moves to `pending_exit`; only a later eligible opportunity returns it to `flat`. An eligible opportunity is a neutral lifecycle transition, **not** a broker fill, price, quantity, execution-feasibility assertion, or economic result. Each `LifecycleRecord` captures the prior state, incoming event reference/time, decision/reason, resulting state, and related immutable action/position evidence.
...
## Implemented P0.4C chronological runner

`shadow.simulation.run_simulation` composes P0.1--P0.4B for the sole current mean-reversion hypothesis. Its immutable `SimulationInput` contains a declared-validated P0.1 bar dataset, one P0.3 `MeanReversionConfig` per configured instrument, price-free P0.4A `ExecutionOpportunity` values, and an optional caller run label. The strategy configuration's required z-score identity and rolling window are the feature configuration; no redundant universal configuration object is introduced.
...
The immutable `SimulationResult` retains metadata plus P0.1 fingerprint, canonical strategy configurations, P0.2 implementation version and snapshots, one structured disposition for every intended decision point, generated P0.3 signals, P0.4A timeline evidence, and P0.4B lifecycle evidence/final state. It intentionally contains no price, quantity, fill, liquidity, cost, cash, P&L, return, or metric. For this baseline, admitting a signal to the research lifecycle is a simulation assumption, not future independent risk authorization. The initial hypothesis retains its threshold exit and does not define a maximum holding horizon, so P0.4C does not manufacture one from `opened_at`; end-of-stream pending and open states stay visible.
