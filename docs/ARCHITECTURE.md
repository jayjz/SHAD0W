...
`shadow.risk` introduces the first operational authorization boundary without changing the research simulator. The authority sequence is `Signal` → provider-neutral `OrderIntent` → pure `RiskDecision` → atomic `RiskGate` admission/reservation → one gate-issued `AuthorizedOrder` → future paper dispatch. `Signal`, `LifecycleAction`, `EconomicExecution`, raw `OrderIntent`, and historical `RiskDecision` values cannot cross the supported dispatch seam.
