...
P0.4B makes the resulting lifecycle state executable without adding economic execution semantics. The dedicated lifecycle component, rather than a strategy, feature engine, or timeline, authoritatively owns one isolated state per instrument: `flat`, `pending_entry`, `holding`, or `pending_exit`. A P0.4A-eligible later opportunity may open or close that state but is not a fill-price claim. Its immutable trace records prior state, event, decision, reason, resulting state, and related action/position evidence. Redundant or impossible signals are explicit non-actions, same-instant opposing entry/exit proposals reject both, and pending/open final states remain visible rather than being silently canceled or liquidated. The lifecycle records `opened_at` for future modeled holding-horizon policy, but P0.4B supplies neither a horizon configuration nor a timeout trigger; P0.4C must compose the required chronological context.
